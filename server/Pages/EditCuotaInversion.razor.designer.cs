...
{
    public partial class EditCuotaInversionComponent : ComponentBase
    {
        ...
        [Parameter]
        public dynamic IdCuotaInversion { get; set; }
        ...
        protected async System.Threading.Tasks.Task Load()
        {
            ...
            var joseGetCuotaInversionByIdCuotaInversionResult = await Jose.GetCuotaInversionByIdCuotaInversion(IdCuotaInversion);
            cuotainversion = joseGetCuotaInversionByIdCuotaInversionResult;
        ...
        protected async System.Threading.Tasks.Task Form0Submit(FinalProyect2.Models.Jose.CuotaInversion args)
        {
            try
            {
                var joseUpdateCuotaInversionResult = await Jose.UpdateCuotaInversion(IdCuotaInversion, cuotainversion);
                DialogService.Close(cuotainversion);
            }
            catch (System.Exception joseUpdateCuotaInversionException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to update CuotaInversion" });

            hasChanges = joseUpdateCuotaInversionException is DbUpdateConcurrencyException;

            if (!(joseUpdateCuotaInversionException is DbUpdateConcurrencyException)) {
                canEdit = false;
            }
            }
        }
