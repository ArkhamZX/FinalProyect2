...
{
    public partial class EditCuentaBancoComponent : ComponentBase
    {
        ...
        [Parameter]
        public dynamic IdCuentaBanco { get; set; }
        ...
        protected async System.Threading.Tasks.Task Load()
        {
            ...
            var joseGetCuentaBancoByIdCuentaBancoResult = await Jose.GetCuentaBancoByIdCuentaBanco(IdCuentaBanco);
            cuentabanco = joseGetCuentaBancoByIdCuentaBancoResult;
        ...
        protected async System.Threading.Tasks.Task Form0Submit(FinalProyect2.Models.Jose.CuentaBanco args)
        {
            try
            {
                var joseUpdateCuentaBancoResult = await Jose.UpdateCuentaBanco(IdCuentaBanco, cuentabanco);
                DialogService.Close(cuentabanco);
            }
            catch (System.Exception joseUpdateCuentaBancoException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to update CuentaBanco" });

            hasChanges = joseUpdateCuentaBancoException is DbUpdateConcurrencyException;

            if (!(joseUpdateCuentaBancoException is DbUpdateConcurrencyException)) {
                canEdit = false;
            }
            }
        }
